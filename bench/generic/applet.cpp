    ...
    if (mp_action == NULL)
    {
        ...
        case (MDI):
        {
            ...
            if(windowTitle() != name())
            {
                ...
                action_name = QString("%1").arg(windowTitle());
            }
            ...
            if ( pnode )
            {
                ...
                if (pnode->type == NodeRegister)
                {
                    ...
                    if(QRegExp("[\\w_]+\\[.*\\]").exactMatch(pctrl->bindName()))
                    {
                        ...
                        int lsb,msb;
                        int idx;
                        char *templ[] = {"([\\w_]+)\\[([0-9]+)\\]","([\\w_]+)\\[([\\w_]*)\\]","([\\w_]+)\\[(\\d+):(\\d+)\\]"};
                        int type_i=0;
                        for(;type_i<3;type_i++)
                        {
                            ...
                            {                                
                                ...
                                case 1: // reg[name]
                                {
                                        ...
                                        if(ptmp->size()>0){
                                            ...
                                            field_readonly = ptmp->bitAt(0)->readOnly();
                                            if(ptmp->size()>1)
                                                descr += QString("<p>%1[%2:0]</p>").arg(rx.cap(1)).arg(ptmp->size()-1);
                                        }
                                ...
                                case 2: // reg[n:m]
                                {
                                        ...
                                        if(ptmp->size()>0){
                                            descr = ptmp->bitAt(0)->description();
                                            descr+= QString("<p>%1[%2:%3]</p>").arg(rx.cap(1)).arg(rx.cap(2)).arg(rx.cap(3));
                                            field_readonly = ptmp->bitAt(0)->readOnly();
                                        }
                                    }
                                    break;
                        ...
                        //qDebug()<<pctrl->bindName()<<field_readonly;
